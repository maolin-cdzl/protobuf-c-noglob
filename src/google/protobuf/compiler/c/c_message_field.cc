...
void MessageFieldGenerator::GenerateDescriptorInitializer(io::Printer* printer,int field_idx) const
{
  string addr = "&" + GetPackageName(descriptor_->message_type()->file()->package()) + "->" + FullNameToLower(descriptor_->message_type()->full_name()) + "__descriptor";
  GenerateDescriptorInitializerGeneric(printer, false, "MESSAGE", addr,field_idx);
}
