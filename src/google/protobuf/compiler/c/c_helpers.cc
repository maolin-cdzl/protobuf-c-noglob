...
int GetIntRangesCount(int n_values,const int* values) {
  if (n_values > 0) {
    int n_ranges = 1;
...
unsigned
WriteIntRanges(io::Printer* printer, int n_values, const int *values, const string &name,const string& package_name)
{
	...
	if (n_values > 0) {
		...
		for (int i = 1; i < n_values; i++) {
			if (values[i-1] + 1 != values[i]) {
				...
				vars["range_count"] = SimpleItoa(range_count);

				printer->Print (vars,"\t$package_name$->$name$[$range_count$].start_value = $start_value$;\n");
				printer->Print (vars,"\t$package_name$->$name$[$range_count$].orig_index = $orig_offset$;\n");
				++range_count;
		...
		printer->Print (vars,"\t$package_name$->$name$[$range_count$].start_value = 0;\n");
		printer->Print (vars,"\t$package_name$->$name$[$range_count$].orig_index = $n_entries$;\n");
		++range_count;
...
string GetPackageName(const string& package) {
	string result;
