...
void BytesFieldGenerator::GenerateDefaultValueImplementations(io::Printer* printer) const
{
  ...
  vars["lcclassname"] = FullNameToLower(descriptor_->containing_type()->full_name());
  vars["default_value_data"] = FullNameToLower(descriptor_->full_name())
	                     + "__default_value_data";
  vars["escaped"] = CEscape(descriptor_->default_value_string());
//  printer->Print(vars, "uint8_t $default_value_data$[] = \"$escaped$\";\n");
  printer->Print(vars,"\t$package_name$->$default_value_data$[] = \"$escaped$\";\n");
}
...
void BytesFieldGenerator::GenerateStaticInit(io::Printer* printer) const
{
  switch (descriptor_->label()) {
    case FieldDescriptor::LABEL_REQUIRED:
	  ...
	  printer->Print(variables_, "message->$name$$deprecated$.len = 0;\n");
	  printer->Print(variables_, "message->$name$$deprecated$.data = NULL;\n");
      break;
    case FieldDescriptor::LABEL_OPTIONAL:
      //printer->Print(variables_, "0,$default_value$");
	  printer->Print(variables_, "message->has_$name$$deprecated$ = 0;\n");
	  printer->Print(variables_, "message->$name$$deprecated$.len = 0;\n");
	  printer->Print(variables_, "message->$name$$deprecated$.data = NULL;\n");
      break;
    case FieldDescriptor::LABEL_REPEATED:
	  ...
	  printer->Print(variables_, "message->n_$name$$deprecated$ = 0;\n");
	  printer->Print(variables_, "message->$name$$deprecated$.len = 0;\n");
	  printer->Print(variables_, "message->$name$$deprecated$.data = NULL;\n");
      break;
  }
}
void BytesFieldGenerator::GenerateDescriptorInitializer(io::Printer* printer,int field_idx) const
{
  GenerateDescriptorInitializerGeneric(printer, true, "BYTES", "NULL",field_idx);
}
