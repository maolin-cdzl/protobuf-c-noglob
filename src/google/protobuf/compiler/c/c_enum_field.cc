...
//      printer->Print(variables_, "$default$");
	  printer->Print(variables_, "message->$name$$deprecated$ = $default$;\n");
      break;
    case FieldDescriptor::LABEL_OPTIONAL:
...
//      printer->Print("0,NULL");
	  ...
	  printer->Print(variables_, "message->$name$$deprecated$ = NULL;\n");
      break;
  }
}

void EnumFieldGenerator::GenerateDescriptorInitializer(io::Printer* printer,int field_idx) const
{
  string addr = "&" + GetPackageName(descriptor_->enum_type()->file()->package()) + "->" + FullNameToLower(descriptor_->enum_type()->full_name()) + "__descriptor";
  GenerateDescriptorInitializerGeneric(printer, true, "ENUM", addr,field_idx);
}
