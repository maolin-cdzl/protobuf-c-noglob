...
//  printer->Print(vars, "char $default$[] = \"$escaped$\";\n");
  ...
  printer->Print(vars,"\t$package_name$->$default$[] = \"$escaped$\";\n");
}
...
void StringFieldGenerator::GenerateStaticInit(io::Printer* printer) const
{
  ...
  switch (descriptor_->label()) {
    case FieldDescriptor::LABEL_REQUIRED:
    case FieldDescriptor::LABEL_OPTIONAL:
	  printer->Print(vars, "message->$name$ = $default$;\n");
      break;
    case FieldDescriptor::LABEL_REPEATED:
//      printer->Print(vars, "0,NULL");
	  printer->Print(vars, "message->n_$name$ = 0;\n");
	  printer->Print(vars, "message->$name$ = NULL;\n");
	  
      break;
  }
}
void StringFieldGenerator::GenerateDescriptorInitializer(io::Printer* printer,int field_idx) const
{
  GenerateDescriptorInitializerGeneric(printer, false, "STRING", "NULL",field_idx);
}
