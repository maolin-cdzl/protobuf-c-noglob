...
//      printer->Print(vars, "$default_value$");
	  printer->Print(vars, "message->$name$ = $default_value$;\n");
      break;
    case FieldDescriptor::LABEL_OPTIONAL:
//      printer->Print(vars, "0,$default_value$");
	  printer->Print(vars, "message->has_$name$ = 0;\n");
	  printer->Print(vars, "message->$name$ = $default_value$;\n");
      break;
    case FieldDescriptor::LABEL_REPEATED:
...
void PrimitiveFieldGenerator::GenerateDescriptorInitializer(io::Printer* printer,int field_idx) const
{
  string c_type_macro;
  switch (descriptor_->type()) {
  ...
  GenerateDescriptorInitializerGeneric(printer, true, c_type_macro, "NULL",field_idx);
}
